...
class Bullet {
    constructor() {
        goalxspeed = ((PLAYER.x + 10 + vx - 250) / 90)
        goalyspeed = ((PLAYER.y + 10 + vy - 250) / 90)
        this.x = 250
        this.y = 250
        bullets.push(this)
    }
...
function drawBackground() {
    ...
    canvasContext.drawImage(level1, 100, 100, 410, 410)
}
...
function onCanvasKeyDown(event) {
    ...
    if (wincheck === 1 && (event.key === "Spacebar " || event.key === ' ')) {
        window.location.href = '../level2/Submarine.html';
    }
}
